...
export default async function HomePage() {
  ...
  return (
    <>
      <Hero spotlightShops={featured} />

      <Section
        title="Boutiques en vedette"
        description="Mises en avant sur owo.bj — bientôt disponible en option payante pour les vendeurs."
        href="/shops"
      >
        {featured.length === 0 ? (
          <EmptyState
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {categoryTiles.map((cat) => (
              <Link
                ...
                className="group flex items-center justify-between gap-3 rounded-xl border border-border bg-surface px-4 py-4 text-body font-medium text-ink transition-colors hover:border-brand-500 hover:bg-surface-subtle"
              >
